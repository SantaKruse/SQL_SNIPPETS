...
This stored procedure is designed to take an NVARCHAR(100) table name as input and autonomously work through each field of the table to remove 'NULL' (NULL stored as text) and '' (empty string).  You do not declare the individual field names that require processing, the procedure automatically loops from the first to the last ordinal field position.
